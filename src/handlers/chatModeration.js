...
module.exports = async function (message, guild = undefined) {
    ...
    if (linkifyReturn.length != 0)for (let index = 0; index < linkifyReturn.length; index++) {
        const element = linkifyReturn[index];
        violations.push({check: `linkify`, trigger: element.type, value: element.value});
    }
    let antiProfanityReturn = antiProfanity.isProfane(message.content, true);
    if (antiProfanityReturn)violations.push({check: `antiProfanity`, trigger: `profanity`});

    let customDetect = detectProfanities(message);
    if (customDetect.length != 0)for (let index = 0; index < customDetect.length; index++) {
        const element = customDetect[index];
        violations.push({check: `custom`, trigger: element.trigger, value: element.value});
    }
...
function detectProfanities(message) {
    ...
    let toCheck = [content];
    let splitters = [" ", ",", ".", ";", ":", "/", "\\", "-", "_", "+", "*", "="];
    let replacePatterns = [["4", "a"], ["8", "b"], ["0", "d"], ["3", "e"], ["1", "i"], ["7", "l"], ["0", "o"], ["5", "s"], ["7", "t"], ["$", "s"], ["@", "a"]];
    let checkWords = {
    ...
    toCheck.forEach(element => {let withoutAccents = removeAccents(element); if (!toCheck.includes(withoutAccents))toCheck.push(withoutAccents)});
    for (const key in checkWords) {
        toCheck.forEach(element => {
            checkWords[key].forEach(el => {
                let violation = {check: `customProfanity`, trigger: key, value: el};
                if (element.includes(el))if (!violations.some(e => (e.check == violation.check && e.value == violation.value)))violations.push(violation);
            })
        })
    }
    return violations;
