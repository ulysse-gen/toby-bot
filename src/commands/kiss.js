...
module.exports = {
    ...
    async execute(CommandExecution) {
        ...
        let MessageToSend = `<@${CommandExecution.Executor.id}> sent a kiss ${possibilities[Math.floor(Math.random()*possibilities.length)]}`;;
        if (!CommandExecution.IsSlashCommand && CommandExecution.Trigger.mentions.members.size != 0) MessageToSend = `<@${CommandExecution.Executor.id}> kissed <@${CommandExecution.Trigger.mentions.members.first().user.id}> ${possibilities[Math.floor(Math.random()*possibilities.length)]}`;
