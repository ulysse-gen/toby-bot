...
module.exports = {
    ...
    async exec(client, message, args, guild = undefined, isSlashCommand = false) {
        ...
        if (result.errored == true) return utils.sendError(message, guild, `Could not ban user`, `${result.reason}`, [], (isSlashCommand) ? {ephemeral: true} : true); /*Updated To New Utils*/
        return (!isSlashCommand) ? true : utils.sendSuccess(message, guild, `User banned.`, undefined, undefined, {ephemeral: true});
    }
}