...
module.exports = {
    ...
    async execute(CommandExecution) {
        ...
        let MessageToSend = `Get slapped by <@${CommandExecution.Executor.id}> ${possibilities[Math.floor(Math.random()*possibilities.length)]}`;;
        if (!CommandExecution.IsSlashCommand && CommandExecution.Trigger.mentions.members.size != 0) MessageToSend = `<@${CommandExecution.Executor.id}> slapped <@${CommandExecution.Trigger.mentions.members.first().user.id}> ${possibilities[Math.floor(Math.random()*possibilities.length)]}`;
