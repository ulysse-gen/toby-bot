...
module.exports = {
    name: "slap",
    description: `Send a slap`,
    aliases: [],
    permission: `commands.slap`,
    nestedPermissions: {
        tag: `commands.slap.tag`,
        reply: `commands.slap.reply`
    },
    category: `fun`,
    cooldown: 45,
    globalCooldown: 30,
    async exec(client, message, args, guild = undefined, isSlashCommand = false) {
        let possibilities = await axios.get('https://g.tenor.com/v1/search?q=slap%20anime&key=LIVDSRZULELA&limit=15').then(data => {
            return data.data.results.map(data => {
                if (!data.itemurl.includes("double") && !data.itemurl.includes("nigg")) return data.url;
            });
        }).catch(error => {
            utils.catchCustomLog(message, guild, e, `Could not fetch slap gifs.`);
            return [];
        });

        if (possibilities.length <= 0)return utils.sendError(message, guild, `Could not get gif.`);

        let sendThis = `Get slapped by <@${message.author.id}> ${possibilities[Math.floor(Math.random()*possibilities.length)]}`;
        ...
        if (hasPermission)
            if (message.mentions.members.size != 0) sendThis = `<@${message.author.id}> slapped <@${message.mentions.members.first().user.id}> ${possibilities[Math.floor(Math.random()*possibilities.length)]}`;
