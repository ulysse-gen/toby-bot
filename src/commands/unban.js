...
module.exports = {
    ...
    async exec(client, message, args, guild = undefined, isSlashCommand = false) {
        ...
        if (result.errored == true) return utils.sendError(message, guild, `Could not unban user`, `${result.reason}`, [], (isSlashCommand) ? {ephemeral: true} : true); /*Updated To New Utils*/
        utils.sendSuccess(message, guild, `User unbanned`, undefined, [], (isSlashCommand) ? {ephemeral: true} : true); /*Updated To New Utils*/
        return true;
    }
}