...
module.exports = {
    name: "notes",
    description: `List the notes for a user.`,
    aliases: [],
    permission: `commands.notes`,
    category: `moderation`,
    async exec(client, message, args, guild = undefined) {
        ...
        let makeTheStats = new Promise((res, rej) => {
            guild.moderationManager.sqlPool.getConnection((err, connection) => {
                ...
                connection.query(`SELECT * FROM \`moderationLogs\` WHERE \`userId\`='${user.user.id}' AND status!='deleted' AND type='Note'`, async function (error, results, fields) {
                    if (results.length == 0) {
                        try {
                            connection.release()
                        } catch (e) {}
        ...
        if (embedFields.length == 0) {
            embed.description = `This member has no notes.`;
            message.reply({
                embeds: [embed],
        ...
        embed.footer = {
            text: `Use \`${guild.configurationManager.configuration.prefix}notes <user> [page number]\` to search thru pages. [1/${embedPages.length}]`
        };
        ...
        if (args.length == 2) {
            ...
            embed.footer = {
                text: `Use \`${guild.configurationManager.configuration.prefix}notes <user> [page number]\` to search thru pages. [${args[1]}/${embedPages.length}]`
            };
