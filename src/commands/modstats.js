...
module.exports = {
    ...
    async exec(client, message, args, guild = undefined) {
        ...
        if (args.length != 0) {
            ...
            if (typeof user == "undefined") return utils.sendError(message, guild, `Could not get moderator data`, `User not found`);
        }
        ...
        if (user.user.id == "280063634477154306")embed.addField(`**Important infos:**`, `Whatever the stats can be, Kilo is still a very bad mod.`, true);
        if (user.user.id == "737886546182799401")embed.addField(`**Important infos:**`, `Wait hm.. I’m not a kitten.`, true);
