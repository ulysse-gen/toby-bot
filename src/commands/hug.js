...
module.exports = {
    ...
    async execute(CommandExecution) {
        ...
        let MessageToSend = `<@${CommandExecution.Executor.id}> sent a hug ! ${possibilities[Math.floor(Math.random()*possibilities.length)]}`;;
        if (!CommandExecution.IsSlashCommand && CommandExecution.Trigger.mentions.members.size != 0) MessageToSend = `Hey <@${CommandExecution.Trigger.mentions.members.first().user.id}>, <@${CommandExecution.Executor.id}> sent you a hug ${possibilities[Math.floor(Math.random()*possibilities.length)]}`;
