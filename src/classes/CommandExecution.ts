...
import { MessageEmbed, GuildMember, Channel, Guild as DiscordGuild, User as DiscordUser, TextChannel } from "discord.js";
...
export default class CommandExecution {
    ...
    async returnRaw(...args){
        var returnOptions = Object.assign({ephemeral: true, slashOnly: false, followUpIfReturned: false}, args[0]) as ReturnOptions;
        if (returnOptions.slashOnly && !this.IsSlashCommand)return true;
    ...
    async sendRaw(...args){
        return (<any>this.Channel.send)(...args).then(message => message);
    }
}