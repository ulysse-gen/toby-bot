...
module.exports = class User {
    ...
    async initialize(createIfNonExistant = false) {
        this.ConfigurationManager = new SQLConfigurationManager(this.UserManager.TobyBot.TopConfigurationManager.get('MySQL'), 'users', `\`id\` = '${this.user.id}'`, undefined, JSON.stringify(require('../../configurations/defaults/UserConfiguration.json')));
        ...
        return new Promise((res, _rej) => {
            this.UserManager.SQLPool.query(`SELECT * FROM \`users\` WHERE id='${this.user.id}'`, (error, results) => {
                ...
                if (results.length != 0){
                    ...
                    this.authToken = results[0].authToken;
