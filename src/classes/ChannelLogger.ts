...
import { UnknownError, TypeError } from './Errors';
...
export default class ChannelLogger {
    ...
    initialized: boolean;
    channel: any;
    constructor(Guild: Guild, channelLoggingConfig) {
    ...
    async initialize() {
        ...
        return this.Guild.Guild.channels.fetch(this.Config.channel).then(async channel => {
            ...
            throw new UnknownError(`Could not initialize ChannelLogger`, {cause: e});
        });
    }
    ...
    async log(content) {
        ...
        if (typeof content != "string" || content.replaceAll(" ", "") == "") throw new TypeError(`Content must be a non empty string.`);
