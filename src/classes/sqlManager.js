...
module.exports = class sqlManager {
    ...
    async checkForExpiredModeration() {
        return this.sqlPool.getConnection((err, connection) => {
            ...
            connection.query(`SELECT * FROM \`moderationLogs\` WHERE \`status\`='active'`, async function (error, results, _fields) {
                ...
                if (typeof results != "undefined" && results != null && results.length != 0) results.forEach(async indPunishments => {
                    if (moment(indPunishments.expires).isBefore(moment())) {
                        await this.client.guilds.fetch(indPunishments.guildId).then(async fetchedGuild => {
                            if (indPunishments.type == "Ban") {
                                await fetchedGuild.bans.remove(indPunishments.userId, `Punishment expire. (Was banned for ${indPunishments.reason})`).then(async () => {
                                    connection.query(`UPDATE \`moderationLogs\` SET \`status\`='expired', \`updaterId\`='${this.client.user.id}', \`updateReason\`='expired', \`updateTimestamp\`='${moment().format(`YYYY-MM-DD HH:mm-ss`)}' WHERE \`numId\`=${indPunishments.numId}`, async function (error, results, fields) {});
                                    MainLog.log(`Unbanned ${(await this.client.users.fetch(indPunishments.userId)).tag}(${indPunishments.userId}). Punishment expired`);
                                }).catch(e => {
                            ...
                            if (indPunishments.type == "Mute" && typeof this.globalGuilds.guilds[indPunishments.guildId] != "undefined") {
                                let muteRole = this.globalGuilds.guilds[indPunishments.guildId].configuration.moderation.muteRole;
                        ...
                        }).catch(e => console.log(`sqlManager.js could not fetch guild ${this.globalGuilds.guilds[indPunishments.guildId]} (${e.toString()})`));
                    }
                    control++;
                    if (control <= 0) {
                        return true;
