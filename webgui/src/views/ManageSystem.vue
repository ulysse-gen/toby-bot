<template>
  <div class="main">
    <section v-if="!loaded" class="full-width loading-anim">
      <h2 class="section-title">Loading bot data..</h2>
    </section>
    <section v-if="loaded" class="full-width server-details">
      <h2 class="section-title">
        Managing
        <span id="guildName" class="server-name">TobyBot</span>
      </h2>
    </section>

    <section v-if="loaded" class="half-width server-config">
      <h2 class="section-title">TobyBot configuration</h2>
...
export default defineComponent({
  ...
  created() {
    fetch(
      `${location.protocol}//${process.env["VUE_APP_TOBYBOT_API_HOST"]}:${process.env["VUE_APP_TOBYBOT_API_PORT"]}/v1/system/configuration`,
      {
        headers: {
      ...
      .then(() => {
        fetch(
          `${location.protocol}//${process.env["VUE_APP_TOBYBOT_API_HOST"]}:${process.env["VUE_APP_TOBYBOT_API_PORT"]}/v1/documentation/configuration/system`,
          {
            headers: {
  ...
  methods: {
    ...
    updateConfiguration(configuration: any) {
      ...
      fetch(
        `${location.protocol}//${process.env["VUE_APP_TOBYBOT_API_HOST"]}:${process.env["VUE_APP_TOBYBOT_API_PORT"]}/v1/system/configuration/${configuration.path}`,
        requestOptions
      ).then((response: any) => {
